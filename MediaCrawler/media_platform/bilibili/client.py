...
class BilibiliClient(AbstractApiClient, ProxyRefreshMixin):
    ...
    async def get_video_all_comments(
        ...
        while not is_end and len(result) < max_count:
            ...
            # =========================================================================
            # 【核心防御性补丁开始】
            # 防止因被风控、无评论或视频关闭评论区导致 replies 返回 null，从而引发 TypeError
            # =========================================================================
            if comment_list is None:
                utils.logger.warning(f"[BilibiliClient.get_video_all_comments] Replies is null (maybe blocked or comments disabled) for video_id: {video_id}. Skipping this video.")
                comment_list = []
                break # 直接退出当前视频的评论循环
            # =========================================================================
