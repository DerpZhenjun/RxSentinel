...
class BilibiliCrawler(AbstractCrawler):
        ...
        for keyword in config.KEYWORDS.split(","):
            ...
            while (page - start_page + 1) * bili_limit_count <= config.CRAWLER_MAX_NOTES_COUNT:
                ...
                for video_item in video_items:
                    if video_item:
                        _view = video_item.get("View")
                        video_id_list.append((str(_view.get("aid")), str(_view.get("bvid") or "")))
    ...
    async def batch_get_video_comments(self, video_id_list: List):
        """
        batch get video comments
        :param video_id_list: list of (avid, bvid) tuples or plain avid strings
        :return:
        """
        ...
        for item in video_id_list:
            if isinstance(item, tuple):
                video_id, bvid = item[0], item[1]
            else:
                video_id, bvid = str(item), ""
            task = asyncio.create_task(self.get_comments(video_id, bvid, semaphore), name=video_id)
            task_list.append(task)
        await asyncio.gather(*task_list)

    async def get_comments(self, video_id: str, bvid: str, semaphore: asyncio.Semaphore):
        """
        get comment for video id
        :param video_id: avid (numeric string)
        :param bvid: BV ID string (e.g. BVxxxxxxxx), used to construct correct video URL
        :param semaphore:
        :return:
        """
        async def _store_callback(vid: str, comments: list):
            await bilibili_store.batch_update_bilibili_video_comments(vid, comments, bvid=bvid)
    ...
    async def get_specified_videos(self, video_url_list: List[str]):
        ...
        for video_detail in video_details:
            if video_detail is not None:
                ...
                video_bvid: str = str(video_item_view.get("bvid") or "")
                if video_aid:
                    video_aids_list.append((str(video_aid), video_bvid))
